...
ACP モードはターミナル UI と同じ永続ワークスペースセッション、ルート制限ツール、計画ワークフロー、サブエージェント、学習、コミットワークフローを使います。接続クライアントへ `/mode`、`/plan`、`/commit`、`/subagents`、`/subagent`、`/learn`、`/clear`、`/help` などを通知します。委任モードでは子の進行状況とツール呼び出しも送ります。
