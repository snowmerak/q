...
// StreamingChatClient supplies streamed model responses.
type StreamingChatClient = streamingChatClient

// StreamChatWithEmptyResponseRecovery applies Q's stream recovery policy.
func StreamChatWithEmptyResponseRecovery(ctx context.Context, configuredClient ChatClient, request client.ChatRequest, emit func(AgentStreamDelta) bool) (*client.ChatResponse, error) {
	return streamChatWithEmptyResponseRecovery(ctx, configuredClient, request, emit)
}

// StreamChatWithConversationRecovery retries a missing streamed rollout.
func StreamChatWithConversationRecovery(ctx context.Context, configuredClient ChatClient, request client.ChatRequest, emit func(AgentStreamDelta) bool) (*client.ChatResponse, error) {
	return streamChatWithConversationRecovery(ctx, configuredClient, request, emit)
}

// ConsumeChatStream joins provider chunks into one response.
func ConsumeChatStream(ctx context.Context, configuredClient StreamingChatClient, request client.ChatRequest, emit func(AgentStreamDelta) bool) (*client.ChatResponse, bool, error) {
	return consumeChatStream(ctx, configuredClient, request, emit)
}
