...
// KnownSkillIDs reconstructs candidates already shown in a transcript.
func KnownSkillIDs(messages []client.Message) map[string]struct{} { return knownSkillIDs(messages) }

// AppendSkillHintContext attaches candidate metadata to one message.
func AppendSkillHintContext(message client.Message, hints *SkillHintSet) client.Message {
	return appendSkillHintContext(message, hints)
}
