...
delegate(subagent_name, prompt) -> TaskResult
```
...
내부 처리기다. ACP Search와 External Web Tester는 agent capability 또는 plan executor다.
이들은 일반 delegation builtin으로 등록하지 않는다.

## 공통 lifecycle

일반 delegation agent는 동일한 `task_start`와 `task_complete` 계약을 사용한다.
...
```text
builtin/scout    -> []
builtin/griller  -> [builtin/scout]
builtin/planner  -> [builtin/scout]
builtin/reviewer -> [builtin/scout]
...
`/subagents` 화면은 builtin과 custom agent를 같은 목록에서 보여 주되 builtin은
read-only로 표시한다. `builtin/coder`에는 workspace mutation 표시를 붙인다. Custom
profile 편집 폼에는 Tools 다음에 Delegates 다중 선택 필드를 추가한다.
...
`/subagents list`와 `/subagents show`는 source, model role, 변경 가능 여부, 도구 및
delegation 정보를 표시한다. `/subagent`는 공개 builtin의 canonical ID와 기존 custom
profile 이름을 실행할 수 있다.
