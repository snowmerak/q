...
func TestModelConfigRoundTripAndClear(t *testing.T) {
	...
	want := ModelConfig{Overrides: map[string]ModelOverride{
		"default": {Model: "local/qwen", ContextWindow: 131072},
...
func TestModelConfigValidation(t *testing.T) {
	...
	for _, value := range []ModelConfig{
		...
		{Overrides: map[string]ModelOverride{"default": {Model: "valid", ContextWindow: -1}}},
...
func TestLoadModelConfigRejectsUnknownFields(t *testing.T) {
	...
	if err := os.WriteFile(store.ModelPath(), []byte(`{"version":1,"overrides":{"default":{"model":"test"}},"unknown":true}`), 0o600); err != nil {
		t.Fatal(err)
	}
